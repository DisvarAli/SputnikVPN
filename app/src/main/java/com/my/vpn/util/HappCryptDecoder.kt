package com.my.vpn.util

import android.content.Context
import android.util.Base64
import com.google.gson.JsonParser
import org.bouncycastle.jce.provider.BouncyCastleProvider
import java.nio.charset.StandardCharsets
import java.security.KeyFactory
import java.security.PrivateKey
import java.security.Security
import java.security.spec.PKCS8EncodedKeySpec
import java.security.spec.RSAPrivateCrtKeySpec
import java.util.concurrent.ConcurrentHashMap
import javax.crypto.Cipher
import javax.crypto.spec.IvParameterSpec

/**
 * Дешифратор Happ crypt…/crypt5 (порт логики [happ-decryptor](https://github.com/LeeeeT/happ-decryptor)).
 * Ключи crypt5 — assets/happ/expanded_rsa_keys.json.
 */
object HappCryptDecoder {

    private val pkcs1KeysB64 = arrayOf(
        "",
        "",
        "",
        ""
    )

    private var crypt5Keys: Map<String, String>? = null
    private val keyCache = ConcurrentHashMap<String, PrivateKey>()

    init {
        if (Security.getProvider(BouncyCastleProvider.PROVIDER_NAME) == null) {
            Security.addProvider(BouncyCastleProvider())
        }
    }

    fun decrypt(link: String, context: Context? = null): String? = runCatching {
        val path = link.trim().removePrefix("happ://")
        when {
            path.startsWith("crypt5/", ignoreCase = true) ->
                decryptCrypt5(path.substring(7), context)
            path.startsWith("crypt4/", ignoreCase = true) ->
                decryptCrypt1to4(3, path.substring(7))
            path.startsWith("crypt3/", ignoreCase = true) ->
                decryptCrypt1to4(2, path.substring(7))
            path.startsWith("crypt2/", ignoreCase = true) ->
                decryptCrypt1to4(1, path.substring(7))
            path.startsWith("crypt/", ignoreCase = true) ->
                decryptCrypt1to4(0, path.substring(6))
            else -> null
        }
    }.getOrNull()

    private fun loadCrypt5Keys(context: Context?): Map<String, String> {
        crypt5Keys?.let { return it }
        val ctx = context ?: return emptyMap()
        val json = ctx.assets.open("happ/expanded_rsa_keys.json").bufferedReader().use { it.readText() }
        val map = mutableMapOf<String, String>()
        JsonParser.parseString(json).asJsonObject.entrySet().forEach { (k, v) ->
            map[k] = v.asString
        }
        crypt5Keys = map
        return map
    }

    private fun decryptCrypt5(payload: String, context: Context?): String {
        val shuffled = blockPairSwap(payload)
        require(shuffled.length >= 8) { "crypt5 payload too short" }
        val marker = shuffled.take(4) + shuffled.takeLast(4)
        val body = shuffled.substring(4, shuffled.length - 4)
        require(body.length >= 13) { "crypt5 body too short" }

        val nonceStr = body.take(12)
        val rest = body.drop(12)
        val digitMatch = Regex("^(\\d+)").find(rest) ?: error("crypt5 segment length missing")
        val segmentLen = digitMatch.groupValues[1].toInt()
        val packed = rest.drop(digitMatch.value.length)
        require(packed.length >= 1 + segmentLen) { "crypt5 segment truncated" }

        val urlB64 = packed.substring(1, 1 + segmentLen)
        val encStr = packed.substring(1 + segmentLen)

        val keys = loadCrypt5Keys(context)
        val rsaKeyB64 = keys[marker] ?: error("No RSA key for marker $marker")
        val privateKey = loadPkcs8Key(rsaKeyB64)
        val rsaPlainStr = String(rsaDecrypt(privateKey, b64DecodeUrlSafe(encStr)), StandardCharsets.ISO_8859_1)
        val chachaKey = b64DecodeUrlSafe(swapPairs(rsaPlainStr))
        val nonce = nonceStr.toByteArray(StandardCharsets.UTF_8)
        val intermediate = chacha20Poly1305Decrypt(chachaKey, nonce, b64DecodeUrlSafe(urlB64))
        val intermediateStr = String(intermediate, StandardCharsets.UTF_8)
        return String(b64DecodeUrlSafe(swapPairs(intermediateStr)), StandardCharsets.UTF_8)
    }

    private fun decryptCrypt1to4(ordinal: Int, payload: String): String {
        val privateKey = loadPkcs1Key(pkcs1KeysB64[ordinal])
        val keySize = (privateKey as java.security.interfaces.RSAKey).modulus.bitLength() / 8
        val cipherBytes = b64DecodeUrlSafe(payload)
        val out = java.io.ByteArrayOutputStream()
        var offset = 0
        while (offset < cipherBytes.size) {
            val chunk = cipherBytes.copyOfRange(offset, minOf(offset + keySize, cipherBytes.size))
            out.write(rsaDecrypt(privateKey, chunk))
            offset += keySize
        }
        return String(out.toByteArray(), StandardCharsets.UTF_8)
    }

    private fun blockPairSwap(s: String): String {
        val fullLen = s.length - (s.length % 4)
        val sb = StringBuilder()
        var offset = 0
        while (offset < fullLen) {
            sb.append(s, offset + 2, offset + 4)
            sb.append(s, offset, offset + 2)
            offset += 4
        }
        if (fullLen < s.length) sb.append(s.substring(fullLen))
        return sb.toString()
    }

    private fun swapPairs(s: String): String {
        val arr = s.toCharArray()
        var i = 0
        while (i + 1 < arr.size) {
            val t = arr[i]
            arr[i] = arr[i + 1]
            arr[i + 1] = t
            i += 2
        }
        return String(arr)
    }

    private fun b64DecodeUrlSafe(s: String): ByteArray {
        var norm = s.replace('-', '+').replace('_', '/')
        while (norm.length % 4 != 0) norm += "="
        return Base64.decode(norm, Base64.DEFAULT)
    }

    private fun loadPkcs1Key(b64: String): PrivateKey =
        keyCache.getOrPut("pkcs1:$b64") {
            val bytes = Base64.decode(b64.replace("\\s".toRegex(), ""), Base64.DEFAULT)
            val spec = org.bouncycastle.asn1.pkcs.RSAPrivateKey.getInstance(bytes)
            val rsaSpec = RSAPrivateCrtKeySpec(
                spec.modulus,
                spec.publicExponent,
                spec.privateExponent,
                spec.prime1,
                spec.prime2,
                spec.exponent1,
                spec.exponent2,
                spec.coefficient
            )
            KeyFactory.getInstance("RSA").generatePrivate(rsaSpec)
        }

    private fun loadPkcs8Key(b64: String): PrivateKey =
        keyCache.getOrPut("pkcs8:$b64") {
            val bytes = Base64.decode(b64.replace("\\s".toRegex(), ""), Base64.DEFAULT)
            KeyFactory.getInstance("RSA").generatePrivate(PKCS8EncodedKeySpec(bytes))
        }

    private fun rsaDecrypt(privateKey: PrivateKey, cipherBytes: ByteArray): ByteArray {
        val cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding")
        cipher.init(Cipher.DECRYPT_MODE, privateKey)
        return cipher.doFinal(cipherBytes)
    }

    private fun chacha20Poly1305Decrypt(key: ByteArray, nonce: ByteArray, ciphertext: ByteArray): ByteArray {
        val cipher = Cipher.getInstance("ChaCha20-Poly1305", BouncyCastleProvider.PROVIDER_NAME)
        val keySpec = javax.crypto.spec.SecretKeySpec(key, "ChaCha20")
        cipher.init(Cipher.DECRYPT_MODE, keySpec, IvParameterSpec(nonce))
        return cipher.doFinal(ciphertext)
    }
}
